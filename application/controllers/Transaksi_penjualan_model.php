...
class Transaksi_penjualan_model extends CI_Controller
{
    function __construct()
    {
        parent::__construct();
        $this->load->model('transaksi_penjualan');
    ...
    public function index()
    {
        ...
        if ($q <> '') {
            $config['base_url'] = base_url() . 'transaksi_penjualan/index.html?q=' . urlencode($q);
            $config['first_url'] = base_url() . 'transaksi_penjualan/index.html?q=' . urlencode($q);
        } else {
            $config['base_url'] = base_url() . 'transaksi_penjualan/index.html';
            $config['first_url'] = base_url() . 'transaksi_penjualan/index.html';
        }

        $config['per_page'] = 10;
        $config['page_query_string'] = TRUE;
        $config['total_rows'] = $this->transaksi_penjualan->total_rows($q);
        $barang = $this->transaksi_penjualan->get_limit_data($config['per_page'], $start, $q);
